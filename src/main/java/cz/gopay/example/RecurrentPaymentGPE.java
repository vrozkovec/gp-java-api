 ...
 */
public class RecurrentPaymentGPE {
	...
	public static final Boolean PRE_AUTHORIZATION = Boolean.TRUE;
	
	public static final Boolean RECURRENT = Boolean.TRUE;
	public static final String REC_CYCLE = "ON_DEMAND";
	public static final Integer REC_PERIOD = 1;
	...
	public static final String ALLOWED_PAYMENT_CHANNELS = GopayHelper.concatPaymentChannels("cz_gp_c");
	//viz Kody platebnich metod - integracni manual;
	public static final String DEFAULT_PAYMENT_CHANNELS = "cz_gp_c";
	...
	public Long createPayment() throws ServiceException, GopayException {
		...
		try {
			...
			command.setPreAuthorization(PRE_AUTHORIZATION);
	...
	public Long capturePayment(Long paymentSessionId) throws ServiceException, GopayException {
		EPaymentResult result = null;
		try {
			
			AxisEPaymentProviderV2 provider = new AxisEPaymentProviderV2ServiceLocator().getEPaymentServiceV2(new URL(WS));
	
			EPaymentSessionInfo sessionInfo = new EPaymentSessionInfo();
			sessionInfo.setPaymentSessionId(paymentSessionId);
			sessionInfo.setTargetGoId(ESHOP_GOID);			
			GopayHelper.signEPaymentSession(sessionInfo, SECURE_KEY);

			result = provider.capturePayment(sessionInfo);

			if (EPaymentResult.RES_FAILED.equals(result.getResult())) {				
				//status.getResultDescription() - popisuje chybovy stav
				throw new GopayException(GopayException.Reason.OTHER,
						"payment not captured [" + result.getResultDescription() + "] ");
			}
	...
	public Long voidAuthorizedPayment(Long paymentSessionId) throws ServiceException, GopayException {
		EPaymentResult result = null;
		try {
			...
			result = provider.voidAuthorization(sessionInfo);
	...
	public Long performRecurrence(Long parentPaymentSessionId) throws ServiceException, GopayException {
		EPaymentStatus status = null;
		try {
			...
			recurrenceRequest.setParentPaymentSessionId(parentPaymentSessionId);
			recurrenceRequest.setOrderNumber(ORDER_ID + "10");			
			recurrenceRequest.setTotalPrice(TOTAL_PRICE_IN_CENTS + 10);
			recurrenceRequest.setTargetGoId(ESHOP_GOID);
			
			GopayHelper.signERecurrenceRequest(recurrenceRequest, SECURE_KEY);

			status = provider.createRecurrentPayment(recurrenceRequest);

			if (!EPaymentStatus.RESULT_CALL_COMPLETED.equals(status.getResult())) {				
				//status.getResultDescription() - popisuje chybovy stav
				throw new GopayException(GopayException.Reason.OTHER,
						"recurrent payment not created [" + status.getResultDescription() + "] ");
			}
	...
	public static void main(String[] args) {
		...
		try {
			RecurrentPaymentGPE sp = new RecurrentPaymentGPE();
...
//			sp.notify(successElement);
			...
			//vytvoreni nasledne (opakovanee platby - opakovani vytvarime nad zalozenou platbou
			System.out.println("\nPERFORM RECURRENCY [paymentSessionId=" + 3000806835l + "]");
			sp.performRecurrence(3000806835l);


//			//zruseni opakovane platby - opakovani rusime nad zakladajci platbou
//			System.out.println("\nVOID RECURRENCY [paymentSessionId=" + 3000806835l + "]");
//			sp.voidRecurrence(3000806835l);
